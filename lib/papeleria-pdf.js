...
async function llenarSoportePago(datos) {
  ...
  draw(C.cliente,        d.cliente || "");
  draw(C.cc,             d.cc || "");
  draw(C.fecha,          d.fecha || "");
  draw(C.direccion,      d.direccion || "");
  draw(C.telefono,       d.telefono || "");
  draw(C.ciudad,         d.ciudad || "MEDELLIN");
  draw(C.descripcion,    d.descripcion || "");
  draw(C.cant,           "1");
  if (d.valor) {
    draw(C.valor_unitario, "$ " + fmtCop(d.valor));
    draw(C.valor_total,    "$ " + fmtCop(d.valor));
    draw(C.subtotal,       fmtCop(d.valor));
    draw(C.total,          fmtCop(d.valor));
  }
  draw(C.recibi_nombre,  d.cliente || "");
  draw(C.recibi_cc,      d.cc || "");
  draw(C.vendedor,       d.vendedor || "");
